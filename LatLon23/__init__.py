...
class GeoCoord(metaclass=abc.ABCMeta):
    '''
...
def string2geocoord(coord_str, coord_class, format_str = 'D'):
    ...
    format_elements = format_str.split('%')
    separators = [sep for sep in format_elements if sep not in list(format2value.keys())] # E.g. ' ', '_' or ', ' characters
    separators.append('%') # Dummy separator for the final part of the coord_str
    formatters = [form for form in format_elements if form in list(format2value.keys())] # E.g. 'D', 'm', or 'S' characters
